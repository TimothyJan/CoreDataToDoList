...
//  CoreDataToDoList
//
//  Created by Timothy Jan on 11/27/21.
//
...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @objc private func didTapAdd(){
        ...
        alert.addAction(UIAlertAction(title:"Submit",
                                      style: .cancel,
                                      handler:{ [weak self] _ in guard let field = alert.textFields?.first,let text = field.text, !text.isEmpty else{
                                            return
                                        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        sheet.addAction(UIAlertAction(title: "Edit",
                                      ...
                                      handler:  { _ in
                                        ...
                                        alert.addAction(UIAlertAction(title:"Save",
                                                                      style: .cancel,
                                                                      handler:{ [weak self] _ in guard let field = alert.textFields?.first, let newName = field.text, !newName.isEmpty else{
                                                                            return
                                                                        }
        ...
        sheet.addAction(UIAlertAction(title: "Delete",
                                      style: .destructive,
                                      handler:  { [weak self] _ in self?.deleteItem(item: item)}))
